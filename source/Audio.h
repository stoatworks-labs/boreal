...
/**
	...
	buffer. What Boreal needs is the same cut: the full-range level (which
	feeds Audio Flux) and the onset detector (each onset is a substorm),
	including the primed first frame that keeps it from going deaf on a clip
	trigger. `--onset` in brtest is the check that the priming works.
